...
for test_size, valid_size in [(0.15, 0.15)]:
    for rescale_factor in rescale_factors:
        ...
        for gamma in gammavalue:
            ...
            predicted_valid1 = clf1.predict(X_valid)
            ...
            acc_valid1 = metrics.accuracy_score(y_pred=predicted_valid1, y_true=y_valid)
            f1_valid1 = metrics.f1_score(
                y_pred=predicted_valid1, y_true=y_valid, average="macro"
            )
            ...
            candidate1 = {
                "acc_valid_tree": acc_valid1,
                "f1_valid_tree": f1_valid1,
                "gamma": gamma,
            }
            model_candidates.append(candidate)
            model_candidates1.append(candidate1)
            print(candidate)
            print(candidate1)
        ...
        max_valid_f1_model_candidate1 = max(
            model_candidates1, key=lambda x: x["f1_valid_tree"]
        )
        ...
        print(
            "{:15s}\t{}x{}\t{}\t{}:{}\t{:.3f}\t{:.3f}".format('Decision Tree',
                resized_images[0].shape[0],
                resized_images[0].shape[1],
                max_valid_f1_model_candidate1["gamma"],
                (1 - test_size) * 100,
                test_size * 100,
                acc1,
                f11,
