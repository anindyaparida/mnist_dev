...
def test(clf,clf1,X_test,y_test):
    ...
    return {"acc": acc, "f1": f1,"acc1": acc1,"f11": f11}

def get_random_acc(y):
    return len(y)

def run_classification_experiment(classifier, X_train, y_train, X_valid, y_valid, gamma, output_model_file,
                                  skip_dummy=True):
